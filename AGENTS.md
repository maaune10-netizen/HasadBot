...
- **Audit**: every admin op writes `admin_actions` via `log_admin_action` (rollback-safe — a failed INSERT never leaves the shared connection's transaction open; the table has **no FK to users** so audit survives user deletion and admin_id=0 actors work).
