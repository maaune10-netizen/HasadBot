...
- **Admin control panel (V1)**: `GET /api/admin/users?q=&filter=` (safe fields, never passwords), `GET/POST /api/admin/users/{id}` + `/renew` `/revoke` `/unlock` `/homework`, `DELETE /api/admin/users/{id}` (requires `{"confirm":"DELETE"}`), `GET /api/admin/payment-requests`, `POST .../activate {"days"}` `/reject {"reason"}`. All mutations call the **shared services in `hasad_bot/admin_ops.py`** — the same functions the Telegram handlers use (byte-identical user notifications, `log_admin_action` + `admin_trace` on every op). Frontend: tab-based SPA (المستخدمين/النشطين/الأسئلة/الأخطاء/المشتركين/APIs/طلبات الدفع/البث والإعلانات) + user-detail modal with action buttons + typed-DELETE confirm.
- **Messaging (V2)**: `GET /api/admin/broadcast/preview?target=` (count + sample), `POST /api/admin/broadcast {target,text,confirm}` → background job → `GET /api/admin/broadcast/jobs/{job_id}` (progress: sent/failed/skipped); `GET /api/admin/announcements`, `POST .../{atype}/toggle|text|preview|send`. Broadcast loop lives in `admin_ops.send_broadcast` (shared with the Telegram handler); announcements reuse `handlers/announcements.py` directly (templates, filters, 25 msg/s rate limit, 24h spam protection, daily jobs in `main.py` untouched). Send jobs run as background tasks with progress in `admin_ops.SEND_JOBS`.
- **Support/Logs/Backups (V3)**: support conversations (from the `logs` table — same data the Telegram flow uses; derived open/closed by last direction), full history + reply via shared `send_support_reply` (byte-identical to Telegram, audit-logged); logs: 6-file allowlist under `config.log_dir` (no arbitrary paths), streaming reads (bounded memory), **redaction of all known secrets** before serving, per-UID user log via the existing parser, admin audit from `admin_actions`; backups: `run_backup` (db/cv/admin_logs) with a concurrency lock, ok-bool propagation from the export functions, audit only on success, encrypted temp files cleaned.
- **Production safety guard**: `APP_ENV` (default `production`) + `ALLOW_LIVE_TESTS`. `admin_ops.assert_side_effect_safe()` runs at the top of **all 20 side-effecting functions** (admin ops, senders, exports): in `APP_ENV=test` without an explicit `HASAD_DATA_DIR` sandbox it raises `OperationBlocked` — a test harness pointed at production data cannot send/reply/delete/export. Test harnesses MUST set `APP_ENV=test` + `HASAD_DATA_DIR`; production (default) is unaffected. **Never run side-effecting tests against the production dashboard/users — sandbox only.**
- **EX (extract credentials)**: exists as a shared service but is **NOT exposed in the dashboard UI** (see Owner-only design pending).
